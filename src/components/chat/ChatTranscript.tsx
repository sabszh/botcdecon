...
export default function ChatTranscript ({
  ...
  return (
    <div
      ref={chatListRef}
      className={`mt-1 flex-1 min-h-0 overflow-y-auto scroll-touch ${isIOS ? '' : 'no-scrollbar'}`}
      style={transcriptScrollStyle}
    >
      <div className='flex min-h-full flex-col justify-start pb-10 pt-10'>
        {messages.map(m => (
          <div key={m.id} data-msg-id={m.id} className={`mb-3 last:mb-0 flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
